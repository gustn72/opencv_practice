...
using namespace cv;
#define PI 3.1415926
#define VIDEOPATH "data/test.MP4"
#define VIDEOOUTPATH "data/result.avi"
...
int main()
{
        ...
        outputVideo.open(VIDEOOUTPATH, VideoWriter::fourcc('X', 'V', 'I', 'D'),
        ...
        while(cap.isOpened()) {
                ...
                for(i=0; i<SAMPLERATE; i++) { //샘플링 함
                        cap >> image;

                        if(image.cols<=0){ //프레임에 아무것도 없으면 탈출
                          break;
                        }
                        //cout << image.cols << "\n";
                }

                resultimg=background.clone(); //결과 프레임 담을 검은색 배경화면 초기화
                ...
                resize(image,image,Size(image.cols/SAMPLERATIO, image.rows/SAMPLERATIO), 0, 0, INTER_AREA);
